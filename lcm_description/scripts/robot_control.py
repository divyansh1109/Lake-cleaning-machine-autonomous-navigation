...
def imu_callback(msg, cmd_vel_msg, label):
    global angular_velocity_w, kp, ki, kd, error_integral, previous_error, angular_velocity_t
    ...
    error = 0.0
    if (cmd_vel_msg.angular.z == angular_velocity_w or cmd_vel_msg.angular.z == -angular_velocity_w):
        
        if (cmd_vel_msg.angular.z == angular_velocity_w):
            # Determine the desired yaw angle for the next 90-degree turn
            target_yaw = yaw + math.pi / 2 - math.pi / 9
            # Calculate the error between the target yaw and the current yaw
            error = target_yaw - yaw
        elif (cmd_vel_msg.angular.z == -angular_velocity_w):
            # Determine the desired yaw angle for the next 90-degree turn
            target_yaw = yaw - math.pi / 2 + math.pi / 6
            # Calculate the error between the target yaw and the current yaw
            error = target_yaw - yaw
        ...
        angular_velocity_w += (p_term + i_term + d_term) * math.pi / 180 - 0.025
        #if (cmd_vel_msg.angular.z == angular_velocity_t):
         #   cmd_vel_msg.angular.z = angular_velocity_w
            
        #elif (cmd_vel_msg.angular.z == -angular_velocity_t):
         #   cmd_vel_msg.angular.z = -angular_velocity_w
...
def control_robot(polygon_coordinates):
    ...
    rospy.Subscriber('/imu_feedback', Imu, lambda msg: imu_callback(msg, cmd_vel_msg, label))
    ...
    for lat in range(min_lat, max_lat):
        ...
        if lat != (max_lat - 1) and is_inside_polygon(lat, lon, polygon_coordinates):
            ...
            print(f"current_angular: {cmd_vel_msg.angular.z}")
            time.sleep(1.6)
            cmd_vel_msg.linear.x = 0.0
            cmd_vel_msg.angular.z = 0.0
            cmd_vel_pub.publish(cmd_vel_msg)
